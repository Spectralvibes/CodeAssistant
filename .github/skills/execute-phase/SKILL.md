...
if [ "$BRANCHING_STRATEGY" = "milestone" ]; then
  # Get current milestone info from .github/ROADMAP.md
  MILESTONE_VERSION=$(grep -oE 'v[0-9]+\.[0-9]+' .github/ROADMAP.md | head -1 || echo "v1.0")
  MILESTONE_NAME=$(grep -A1 "## .*$MILESTONE_VERSION" .github/ROADMAP.md | tail -1 | sed 's/.*- //' | cut -d'(' -f1 | tr -d ' ' || echo "milestone")
...
` section. Extract what's being built and why it matters.
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   CONFIG_CONTENT=$(cat .github/config.json 2>/dev/null)
   ```
   ...
   Commit each task atomically. Create SUMMARY.md. Update .github/STATE.md.
   ...
   - [ ] .github/STATE.md updated with position and decisions
...
Phase goal: {goal from .github/ROADMAP.md}
...
Update .github/ROADMAP.md to reflect phase completion:
...
```bash
git add .github/ROADMAP.md .github/STATE.md ./phases/{phase_dir}/*-VERIFICATION.md
git add .github/REQUIREMENTS.md  # if updated
...
- Record partial progress in .github/STATE.md
