...
Read .github/STATE.md before any operation to load project context.
Read .github/config.json for planning behavior settings.
...
```bash
cat .github/STATE.md 2>/dev/null
```
...
**If file missing but ./ exists:**

```
.github/STATE.md missing but planning artifacts exist.
Options:
1. Reconstruct from existing artifacts
2. Continue without project state (may lose accumulated context)
```

**If ./ doesn't exist:** Error - project not initialized.
...
ls ./phases/XX-name/*-PLAN.md 2>/dev/null | sort
ls ./phases/XX-name/*-SUMMARY.md 2>/dev/null | sort
```
...
- Integer: `./phases/01-foundation/01-01-PLAN.md`
- Decimal: `./phases/01.1-hotfix/01.1-01-PLAN.md`
...
grep -n "type=\"checkpoint" ./phases/XX-name/{phase}-{plan}-PLAN.md
```
...
2. Use Task tool with subagent_type="executor" and model="{executor_model}":

   Prompt: "Execute plan at ./phases/{phase}-{plan}-PLAN.md
...
3. After Task tool returns with agent_id:
   ...
   a. Write agent_id to current-agent-id.txt:
      echo "[agent_id]" > ./current-agent-id.txt
...
For each autonomous segment:
  Spawn subagent with prompt: "Execute tasks [X-Y] from plan at ./phases/{phase}-{plan}-PLAN.md. Read the plan for full context and deviation rules. Do NOT create SUMMARY or commit - just execute these tasks and report results."
...
if [ ! -f ./agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > ./agent-history.json
fi

# Clear any stale current-agent-id (from interrupted sessions)
# Will be populated when subagent spawns
rm -f ./current-agent-id.txt
```
...
if [ -f ./current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat ./current-agent-id.txt)
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="executor" and model="{executor_model}":
...
```bash
cat ./phases/XX-name/{phase}-{plan}-PLAN.md
````
...
ls ./phases/*/SUMMARY.md 2>/dev/null | sort -r | head -2 | tail -1
```
...
2. For each task:
   ...
   **Before executing:** Check if task has `tdd="true"` attribute:
   - If yes: Follow TDD execution flow (see `## Tdd Execution

`) - RED → GREEN → REFACTOR cycle with atomic commits per stage
   ...
   - **Commit the task** (see `## Task Commit
...
3. Run overall verification checks from `## Verification

` section
4. Confirm all success criteria from `## Success criteria
...
` element for test specification
...
**Note:** TDD plans have their own commit pattern (test/feat/refactor for RED/GREEN/REFACTOR phases). See `## Tdd Plan Execution
...
### Step: checkpoint_return_for_orchestrator
...
```bash
grep -A 50 "^user_setup:" ./phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

**If user_setup exists and is not empty:**

Create `./phases/XX-name/{phase}-USER-SETUP.md` using template from `~/./templates/user-setup.md`.
...
Create `{phase}-{plan}-SUMMARY.md` as specified in the prompt's `## Output

` section.
Use ~/./templates/summary.md for structure.

**File location:** `./phases/XX-name/{phase}-{plan}-SUMMARY.md`
...
Update Current Position section in .github/STATE.md to reflect plan completion.
...
- Count total plans across all phases (from .github/ROADMAP.md or .github/ROADMAP.md)
...
### Step: extract_decisions_and_issues


Extract decisions, issues, and concerns from SUMMARY.md into .github/STATE.md accumulated context.

**Decisions Made:**

- Read SUMMARY.md "## Decisions Made" section
- If content exists (not "None"):
  - Add each decision to .github/STATE.md Decisions table
...
- If contains blockers or concerns:
  - Add to .github/STATE.md "Blockers/Concerns Carried Forward"
    

### Step: update_session_continuity


Update Session Continuity section in .github/STATE.md to enable resumption in future sessions.
...
**Size constraint note:** Keep .github/STATE.md under 150 lines total.


### Step: issues_review_gate
...
```bash
ROADMAP_FILE=".github/ROADMAP.md"
```
...
- Skip all git operations for ./ files
...
```bash
git add ./phases/XX-name/{phase}-{plan}-SUMMARY.md
...
SUMMARY: ./phases/XX-name/{phase}-{plan}-SUMMARY.md
EOF
)"
```
...
SUMMARY: ./phases/08-user-auth/08-02-registration-SUMMARY.md
EOF
)"
```
...
### Step: update_codebase_map


**If .github/codebase/ exists:**
...
```bash
git add .github/codebase/*.md
git commit --amend --no-edit  # Include in metadata commit
```

**If .github/codebase/ doesn't exist:**
Skip this step.
...
📋 ./phases/{phase-dir}/{phase}-USER-SETUP.md
...
Run `cat ./phases/{phase-dir}/{phase}-USER-SETUP.md` for full details.
...
Summary: ./phases/{phase-dir}/{phase}-{plan}-SUMMARY.md
...
Read .github/ROADMAP.md and extract:
...
**Phase {Z+1}: {Next Phase Name}** — {Goal from .github/ROADMAP.md}
...
` — add another phase before completing
...
- .github/STATE.md updated (position, decisions, issues, session)
