...
**Orchestrator role:** Parse arguments, validate phase, research domain (unless skipped or exists), spawn planner agent, verify plans with plan-checker, iterate until plans pass or max iterations reached, present results.
...
ls ./phases/${PHASE}-*/*-PLAN.md 2>/dev/null
```
...
PHASE_DIR=$(ls -d ./phases/${PHASE}-* 2>/dev/null | head -1)
if [ -z "$PHASE_DIR" ]; then
  # Create phase directory from roadmap name
  PHASE_NAME=$(grep "Phase ${PHASE}:" .github/ROADMAP.md | sed 's/.*Phase [0-9]*: //' | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
  mkdir -p "./phases/${PHASE}-${PHASE_NAME}"
  PHASE_DIR="./phases/${PHASE}-${PHASE_NAME}"
fi
...
```bash
WORKFLOW_RESEARCH=$(cat .github/config.json 2>/dev/null | grep -o '"research"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false' || echo "true")
```
...
PHASE_DESC=$(grep -A3 "Phase ${PHASE}:" .github/ROADMAP.md)

# Get requirements if they exist
REQUIREMENTS=$(cat .github/REQUIREMENTS.md 2>/dev/null | grep -A100 "## Requirements" | head -50)

# Get prior decisions from .github/STATE.md
DECISIONS=$(grep -A20 "### Decisions Made" .github/STATE.md 2>/dev/null)
...
**Prior decisions from .github/STATE.md:**
{decisions}
...
ROADMAP_CONTENT=$(cat .github/ROADMAP.md)

# Read optional files (empty string if missing)
REQUIREMENTS_CONTENT=$(cat .github/REQUIREMENTS.md 2>/dev/null)
# CONTEXT_CONTENT already loaded in step 4
...
```
Task(
  prompt="First, read .github/agents/planner.agent.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
- Check config: `WORKFLOW_PLAN_CHECK=$(cat .github/config.json 2>/dev/null | grep -o '"plan_check"[[:space:]]*:[[:space:]]*[^,}]*' | grep -o 'true\|false' || echo "true")`
...
- cat ./phases/{phase-dir}/\*-PLAN.md — review plans
