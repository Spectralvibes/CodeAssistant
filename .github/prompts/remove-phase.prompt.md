...
cat .github/ROADMAP.md 2>/dev/null
```

Parse current phase number from .github/STATE.md "Current Position" section.


### Step: validate_phase_exists


Verify the target phase exists in .github/ROADMAP.md:
...
### Step: validate_future_phase


Verify the phase is a future phase (not started):

1. Compare target phase to current phase from .github/STATE.md
...
```bash
ls ./phases/{target}-*/*-SUMMARY.md 2>/dev/null
```
...
### Step: gather_phase_info


Collect information about the phase being removed:

1. Extract phase name from .github/ROADMAP.md heading: `### Phase {target}: {Name}`
2. Find phase directory: `./phases/{target}-{slug}/`
...
### Step: delete_phase_directory


Delete the target phase directory if it exists:

```bash
if [ -d "./phases/{target}-{slug}" ]; then
  rm -rf "./phases/{target}-{slug}"
  echo "Deleted: ./phases/{target}-{slug}/"
fi
```
...
mv "./phases/18-dashboard" "./phases/17-dashboard"
```
...
### Step: rename_files_in_directories
...
Write updated .github/ROADMAP.md.
...
Write updated .github/STATE.md.


### Step: update_file_contents
...
grep -r "Phase 18" ./phases/17-*/ 2>/dev/null
grep -r "Phase 19" ./phases/18-*/ 2>/dev/null
# etc.
...
- Don't add "removed phase" notes to .github/STATE.md - git commit is the record
...
- Just delete and update .github/ROADMAP.md, no renumbering needed

**Phase directory doesn't exist:**

- Phase may be in .github/ROADMAP.md but directory not created yet
- Skip directory deletion, proceed with .github/ROADMAP.md updates
...
- [ ] .github/ROADMAP.md updated (section removed, all references renumbered)
- [ ] .github/STATE.md updated (phase count, progress percentage)
