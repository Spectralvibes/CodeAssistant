...
- The ## Action
 ...
 section is more than a paragraph
...
After completion, create `./phases/XX-name/{phase}-{plan}-SUMMARY.md`
...
Take the phase goal from .github/ROADMAP.md. This is the outcome, not the work.
...
PHASE_DIR=$(ls -d ./phases/$PADDED_PHASE-* ./phases/$PHASE_ARG-* 2>/dev/null | head -1)
...
Triggered when orchestrator provides `## Revision Context

` with checker issues. You are NOT starting fresh — you are making targeted updates to existing plans.
...
```bash
git add ./phases/$PHASE-*/$PHASE-*-PLAN.md
...
| 16-01 | Added ## Verify
 ...
 to Task 2 | task_completeness              |
| 16-02 | Added logout task        | requirement_coverage (AUTH-02) |

### Files Updated

- ./phases/16-xxx/16-01-PLAN.md
- ./phases/16-xxx/16-02-PLAN.md
...
Read `.github/STATE.md` and parse:
...
If .github/STATE.md missing but ./ exists, offer to reconstruct or continue without.
...
### Step: load_codebase_context


Check for codebase map:

```bash
ls .github/codebase/*.md 2>/dev/null
```
...
### Step: mandatory_discovery


Apply discovery level protocol (see discovery_levels section).


### Step: read_project_history
...
**From .github/STATE.md:** Decisions -> constrain approach. Pending todos -> candidates.


### Step: gather_phase_context
...
PHASE_DIR=$(ls -d ./phases/$PADDED_PHASE-* ./phases/$PHASE-* 2>/dev/null | head -1)
...
### Step: write_phase_prompt


Use template structure for each PLAN.md.

Write to `./phases/XX-name/{phase}-{NN}-PLAN.md` (e.g., `01-02-PLAN.md` for Phase 1, Plan 2)
...
Update .github/ROADMAP.md to finalize phase placeholders created by add-phase or insert-phase.

1. Read `.github/ROADMAP.md`
...
4. Write updated .github/ROADMAP.md
...
```bash
git add ./phases/$PHASE-*/$PHASE-*-PLAN.md .github/ROADMAP.md
...
- ./phases/{phase_dir}/{phase}-{plan}-PLAN.md
...
- [ ] .github/STATE.md read, project history absorbed
