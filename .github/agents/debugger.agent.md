...
- `/debug.md` command (interactive debugging)
...
```
DEBUG_DIR=./debug
DEBUG_RESOLVED_DIR=./debug/resolved
```
...
```bash
ls ./debug/*.md 2>/dev/null | grep -v resolved
```
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /debug.md to resume" if context filling up.


### Step: resume_from_file
...
```bash
mkdir -p ./debug/resolved
mv ./debug/{slug}.md ./debug/resolved/
```
...
```bash
# Only commit code changes, exclude ./
git add -A
git reset ./
git commit -m "fix: {brief description}
...
See ## Checkpoint Behavior

 section for full format.
